...
export const BlogPostPage: React.FC<BlogPostPageProps> = ({ slug, onNavigate, onOpenPost, onOpenContact }) => {
  ...
  useSEO(post ? {
    ...
    jsonLd: [{
      ...
      '@type': 'BlogPosting',
      '@id': `https://flynnjamespontino-porfolio.onrender.com/blog/${post.slug}#article`,
      headline: post.title,
      description: post.excerpt,
      image: [post.image],
      datePublished: post.published,
      dateModified: post.updated,
      mainEntityOfPage: `https://flynnjamespontino-porfolio.onrender.com/blog/${post.slug}`,
      author: { '@id': 'https://flynnjamespontino-porfolio.onrender.com/#person' },
      ...
      '@type': 'BreadcrumbList',
      itemListElement: [
        { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://flynnjamespontino-porfolio.onrender.com/' },
        { '@type': 'ListItem', position: 2, name: 'Blog', item: 'https://flynnjamespontino-porfolio.onrender.com/blog' },
        { '@type': 'ListItem', position: 3, name: post.title, item: `https://flynnjamespontino-porfolio.onrender.com/blog/${post.slug}` },
      ],
    }]
  } : {
      ...
      <Section className="pt-12 sm:pt-16">
        <article className="max-w-4xl mx-auto">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
            ...
            <div className="inline-flex items-center gap-2 text-[11px] font-mono text-slate-500 uppercase tracking-wider"><Clock className="w-3.5 h-3.5" /> {post.readTime}</div>
          </div>

          <div className="relative rounded-2xl overflow-hidden border border-slate-700/70 aspect-[16/7] mb-10">
            <img src={post.image} alt={`${post.title} — Flynn James B2B sales blog`} className="w-full h-full object-cover" fetchPriority="high" decoding="async" />
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          <div className="lg:col-span-7">
            ...
            <div className="mt-6 grid gap-3">
              {related.map((item) => (
                <a key={item.slug} href={`/blog/${item.slug}`} onClick={(event) => { event.preventDefault(); onOpenPost(item.slug); }} className="group text-left rounded-xl border border-slate-700/60 bg-slate-900/60 p-5 hover:border-slate-500 transition-colors block">
                  ...
                  <div className="mt-2 text-[12.5px] text-slate-500 leading-6">{item.excerpt}</div>
                </a>
              ))}
            </div>
          </div>
